		...
		<div class="text-center"> 
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb justify-content-center">
					...
					<? 
						$a=mysqli_query($conn,"select * from menu WHERE id='$_REQUEST[id]'");
$b=mysqli_fetch_array($a);
	$res = mysqli_query($conn, "SELECT * FROM kultura_cxrili where kategory='$b[id]'");
	$data = mysqli_fetch_assoc($res);
	echo $b["title_$_SESSION[LANG]"]; ?>
...
$data=mysqli_fetch_array($res);
	include('video.php');  
			...
			<?
				$res = mysqli_query($conn, "SELECT * FROM kultura_cxrili WHERE kategory='$_GET[id]' AND hidden='0'");
	$b = mysqli_fetch_assoc($res);
			
			if($b['menu']=='1')
			{
			?>
			<div class="col-md-12 col-sm-4 wow fadeInDown" data-wow-delay=".5s" >
				<div class="service-content text-center" style="cursor: pointer; margin-top:-233px;">
						<div class="grid">
 				...
 				<div class="col-md-6" style="text-align: left; align="center">	<span style="float:left"><?php echo $b["full_$_SESSION[LANG]"];?></span> </div>
				</div>
			</div>
			<?
			}
	...
	*/
	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE kategory='$_GET[id]' AND hidden='0'";
	...
	$sql = "SELECT * FROM $tbl_name WHERE kategory='$_GET[id]' AND hidden='0' order by news_date desc LIMIT $start, $limit";
